...
export default function MyRunsScreen({ myRuns, setMyRuns }) {
  const [logging, setLogging] = useState(false);

  const handleAdd = (run) => setMyRuns(runs => [run, ...runs]);
  ...
  return (
    <div className="screen">
      <div className="header">
        <div className="header-top">
          <div>
            ...
            <div className="header-sub">{totalRuns} logged · {pbs.length} PBs</div>
          </div>
      ...
      {logging && <LogRunSheet onClose={() => setLogging(false)} onAdd={handleAdd} />}
    </div>
  );
}
